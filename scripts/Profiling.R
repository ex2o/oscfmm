...
for (GG in 1:GGmax) {
  ...
  K_means_init <- kmeans(Data1$X,centers = GG,iter.max = 100, nstart=nstart, algorithm=alg) # Initialize a clustering using k-means
  ...
  K_means_init <- kmeans(Data2$X,centers = GG,iter.max = 100, nstart=nstart, algorithm=alg)
  ...
  K_means_init <- kmeans(Data1$X,centers = GG+LL,iter.max = 100, nstart=nstart, algorithm=alg)
  ...
  K_means_init <- kmeans(Data2$X,centers = GG+LL,iter.max = 100, nstart=nstart, algorithm=alg)
  param_init <- me('VVV',Data2$X,z=unmap(K_means_init$cluster),control=emControl(itmax=1))
